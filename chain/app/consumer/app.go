...
import (
	...
	minakeyskeeper "github.com/cosmos/interchain-security/v5/x/minakeys/keeper"
...
func New(
	...
	secondaryKey, err := minakeysutils.LoadSecondaryKeyFromHex(hexStr, logger)
	if err != nil {
		panic(err)
	}

	// Initialize MinakeysKeeper
	app.MinakeysKeeper = minakeyskeeper.NewKeeper(
		appCodec,
		runtime.NewKVStoreService(keys[minakeysmoduletypes.StoreKey]),
		logger,
		authtypes.NewModuleAddress(govtypes.ModuleName).String(),
		secondaryKey,
	)
	...
	voteExtOp := func(bApp *baseapp.BaseApp) {
		voteExtHandler := NewVoteExtensionHandler(minaVoteExtHandler)
	...
	bApp.SetPreBlocker(minaVoteExtHandler.PreBlocker())
	...
	app.SetPrepareProposal(minaVoteExtHandler.PrepareProposalHandler())
	app.SetProcessProposal(minaVoteExtHandler.ProcessProposalHandler())
