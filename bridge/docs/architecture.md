...
**`pusher.ts`** — The bridge process owns the periodic `MsgPushNewActions` tx (agreed with the chain team — no external cron): the push is the ignition for the chain's adjudication; the archive-wrapper behind it is a passive query service that never initiates anything. Every `PUSH_INTERVAL_MS` it reads the chain's Mina scan cursor (`LatestActionHashes`) and the module params, observes the Mina tip, and pushes `min(tip, cursor + max_block_range)` — mirroring the keeper's admission checks so a rejected tx means a *race* (someone else advanced the cursor — harmless, the message is permissionless and height-based), a *lagging wrapper* (the wrapper serves confirmed heights and sits its own unqueryable confirmation depth behind the tip, so the pusher keeps a self-tuning tip margin: each rejection widens it a step, a streak of accepted pushes narrows it back), or a *configuration fault* (loud log; out-of-gas lands here too, and notably the genesis deadlock where `cursor + max_block_range` cannot reach `start_block_height` — only a genesis reseed or a param bump fixes that). DeliverTx failures are classified by x/bridge's registered error codes (the bare code is unambiguous — one msg type in the tx, module codes ≥1102, SDK ante codes <100 — which matters because the shipped cosmjs drops the codespace and SDK 0.50+ leaves rawLog empty). Signs with `PULSAR_PRIVATE_KEY_HEX` over Tendermint RPC (`PULSAR_RPC_ENDPOINT`); both set enables the pusher, exactly one is a boot error, neither disables it (lightnet).
