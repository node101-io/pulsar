...
import { BlockData, VoteExt } from "../utils/interfaces.js";
import {
...
export class PulsarClient extends EventEmitter {
    ...
    private async createClient(serviceName: string, rpcAddress: string) {
    ...
    async start() {
        ...
        try {
            this.tmClient = await this.createClient(
                TENDERMINT_SERVICE_NAME,
                this.rpcAddress,
            );
            this.mkClient = await this.createClient(
                MINA_KEYS_SERVICE_NAME,
                this.rpcAddress,
            );
            ...
            logger.error("Error starting Pulsar client, retrying...", error, {
                event: "client_start_error",
            });
    ...
    pollLatestBlock() {
        this.tmClient.GetLatestBlock({}, (err: unknown, res: any) => {
            if (err) {
            ...
            if (height > this.lastSeenHeight + 1) {
                if (height === this.lastSeenHeight + 2) {
                    ...
                    this.syncMissedBlocks().catch((error) =>
                        this.emit("error", error as Error),
                    );
                }
            }
    ...
    async syncMissedBlocks() {
        return new Promise<void>((resolve, reject) => {
            this.tmClient.GetLatestBlock({}, async (err: unknown, res: any) => {
                ...
                logger.info("Syncing missed blocks", {
                    ...
                    blocksToSync:
                        Number(height) - 1 - Number(this.lastSeenHeight),
    ...
    async getMinaPubKeyFromCosmosAddress(
        cosmosAddress: string,
    ): Promise<string> {
        return new Promise((resolve, reject) => {
            this.mkClient.KeyStore(
                { index: cosmosAddress },
                (err: unknown, res: any) => {
                    if (err) return reject(err as Error);
    ...
    async getValidatorSet(height: number): Promise<string[]> {
        try {
            ...
            for (const validator of validators) {
                try {
                    const pubkey = await this.getMinaPubKeyFromCosmosAddress(
                        validator,
                    );
    ...
    async parseVoteExtResponse(res: any): Promise<VoteExt[]> {
        ...
        try {
            for (const v of res.voteExt) {
                const recoveredPubkey = await this.recoverPubkeyFromEncoded(
                    v.validatorAddr,
                );
