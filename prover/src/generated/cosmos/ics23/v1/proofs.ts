...
function createBaseExistenceProof(): ExistenceProof {
  return { key: Buffer.alloc(0), value: Buffer.alloc(0), leaf: undefined, path: [] };
}
...
function createBaseNonExistenceProof(): NonExistenceProof {
  return { key: Buffer.alloc(0), left: undefined, right: undefined };
}
...
function createBaseLeafOp(): LeafOp {
  return { hash: 0, prehashKey: 0, prehashValue: 0, length: 0, prefix: Buffer.alloc(0) };
}
...
function createBaseInnerOp(): InnerOp {
  return { hash: 0, prefix: Buffer.alloc(0), suffix: Buffer.alloc(0) };
}
...
function createBaseInnerSpec(): InnerSpec {
  return { childOrder: [], childSize: 0, minPrefixLength: 0, maxPrefixLength: 0, emptyChild: Buffer.alloc(0), hash: 0 };
}
...
export const InnerSpec: MessageFns<InnerSpec> = {
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 5: {
          ...
          message.emptyChild = Buffer.from(reader.bytes());
          continue;
        }
        case 6: {
          if (tag !== 48) {
  ...
  fromJSON(object: any): InnerSpec {
    return {
      ...
      emptyChild: isSet(object.emptyChild) ? Buffer.from(bytesFromBase64(object.emptyChild)) : Buffer.alloc(0),
      hash: isSet(object.hash) ? hashOpFromJSON(object.hash) : 0,
    };
  },
  ...
  fromPartial<I extends Exact<DeepPartial<InnerSpec>, I>>(object: I): InnerSpec {
    ...
    message.emptyChild = object.emptyChild ?? Buffer.alloc(0);
    message.hash = object.hash ?? 0;
    return message;
  },
};
