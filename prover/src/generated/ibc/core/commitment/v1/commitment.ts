...
function createBaseMerklePrefix(): MerklePrefix {
  return { keyPrefix: Buffer.alloc(0) };
}
...
export const MerklePrefix: MessageFns<MerklePrefix> = {
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1: {
          ...
          message.keyPrefix = Buffer.from(reader.bytes());
          continue;
        }
      }
      if ((tag & 7) === 4 || tag === 0) {
  ...
  fromJSON(object: any): MerklePrefix {
    return { keyPrefix: isSet(object.keyPrefix) ? Buffer.from(bytesFromBase64(object.keyPrefix)) : Buffer.alloc(0) };
  },
  ...
  fromPartial<I extends Exact<DeepPartial<MerklePrefix>, I>>(object: I): MerklePrefix {
    const message = createBaseMerklePrefix();
    message.keyPrefix = object.keyPrefix ?? Buffer.alloc(0);
    return message;
  },
};
